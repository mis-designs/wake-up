...
function doGet(e) {
  try {
    ...
    if (action === "login" || action === "validate") {
      ...
      const registerDevice = action === "login" || isTrue(p.registerDevice);
...
function doPost(e) {
  ...
  try {
    ...
    const registerDevice = action === "login" || isTrue(data.registerDevice);
    const result = handleUserAccess(sheet, phone, deviceId, registerDevice, now);
