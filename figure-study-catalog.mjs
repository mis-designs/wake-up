...
const groups = [
  ...
  ['divieto', 'Divieto', 'নিষেধাজ্ঞার সংকেত', 'Osserva a chi si applica il divieto e gli eventuali pannelli.', '54 55 56 57 59 60 63 64 66 67 68 70 71 72 73 75 76 77 78 79 80 81 82 83 84 85 86 90 91 92 151 942 945 949 955 957'],
  ...
  ['indicazione', 'Indicazione', 'তথ্যসূচক সংকেত', 'Località, percorsi, servizi e informazioni utili alla guida.', '153 166 167 168 172 182 183 184 186 189 193 194 197 199 200 201 203 206 218 219 225 226 228 230 231 236 238 239 240 242 243 250 256 270 273'],
...
export function searchStudyFigures(category, query = '') {
  ...
  return FIGURE_STUDY_ITEMS.filter(item => (!category || item.category === category)
    && words.every(word => normalizeSearch(`${item.italian} ${item.bangla}`).includes(word)));
}
