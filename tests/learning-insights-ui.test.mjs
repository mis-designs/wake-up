...
test("the UI includes stable loading, empty, offline, error and insufficient states", () => {
  ...
  assert.match(client, /requestInsights\(auth, localEvents, controller\.signal, true, force\)/u);
...
test("responsive, reduced-motion and global scrollbar rules are present", () => {
  ...
  assert.match(index, /src\/learning-insights\.js\?v=12-study-results/u);
  assert.match(worker, /magicbook-pwa-v222-liquid-actions/u);
  ...
  assert.match(worker, /src\/learning-insights\.js\?v=12-study-results/u);
...
test("learning workspace keeps a flat, aligned surface rhythm", () => {
  ...
  assert.match(css, /#learningInsightsScreen \.li-main \{\s*width: min\(1720px, calc\(100% - 48px\)\);/u);
