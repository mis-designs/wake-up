...
test("leaving the admin panel restores the entry and ships the new script version", () => {
  ...
  assert.match(htmlSource, /script\.js\?v=56-login-daisyui/);
  assert.match(workerSource, /script\.js\?v=56-login-daisyui/);
  assert.match(workerSource, /CACHE_NAME = "magicbook-pwa-v116-home-learning-layout"/);
});
