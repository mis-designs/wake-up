...
test("leaving the admin panel restores the entry and ships the new script version", () => {
  ...
  assert.match(htmlSource, /script\.js\?v=74-quiz-thumb/);
  assert.match(workerSource, /script\.js\?v=74-quiz-thumb/);
  assert.match(workerSource, /CACHE_NAME = "magicbook-pwa-v197-dictionary-sequence"/);
});
