...
test("leaving the admin panel restores the entry and ships the new script version", () => {
  ...
  assert.match(htmlSource, /script\.js\?v=73-native-mode-pages/);
  assert.match(workerSource, /script\.js\?v=73-native-mode-pages/);
  assert.match(workerSource, /CACHE_NAME = "magicbook-pwa-v186-native-mode-pages"/);
});
