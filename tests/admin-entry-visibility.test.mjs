...
test("leaving the admin panel restores the entry and ships the new script version", () => {
  ...
  assert.match(htmlSource, /script\.js\?v=52-promo-users-tab/);
  assert.match(workerSource, /script\.js\?v=52-promo-users-tab/);
  assert.match(workerSource, /CACHE_NAME = "magicbook-pwa-v109-promo-users-tab"/);
});
