...
test("leaving the admin panel restores the entry and ships the new script version", () => {
  ...
  assert.match(htmlSource, /script\.js\?v=64-admin-control-pills/);
  assert.match(workerSource, /script\.js\?v=64-admin-control-pills/);
  assert.match(workerSource, /CACHE_NAME = "magicbook-pwa-v147-admin-overtime-timer"/);
});
