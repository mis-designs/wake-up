...
test("leaving the admin panel restores the entry and ships the new script version", () => {
  ...
  assert.match(htmlSource, /script\.js\?v=49-chapter-card-titles/);
  assert.match(workerSource, /script\.js\?v=49-chapter-card-titles/);
  assert.match(workerSource, /CACHE_NAME = "magicbook-pwa-v106-chapter-card-titles"/);
});
