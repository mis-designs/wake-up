...
test("leaving the admin panel restores the entry and ships the new script version", () => {
  ...
  assert.match(htmlSource, /script\.js\?v=60-promo-single-use/);
  assert.match(workerSource, /script\.js\?v=60-promo-single-use/);
  assert.match(workerSource, /CACHE_NAME = "magicbook-pwa-v127-promo-single-use"/);
});
