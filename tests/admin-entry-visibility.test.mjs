...
test("leaving the admin panel restores the entry and ships the new script version", () => {
  ...
  assert.match(htmlSource, /script\.js\?v=38-promo-login/);
  assert.match(workerSource, /script\.js\?v=38-promo-login/);
  assert.match(workerSource, /CACHE_NAME = "magicbook-pwa-v71-promo-login"/);
});
