...
test("leaving the admin panel restores the entry and ships the new script version", () => {
  ...
  assert.match(htmlSource, /script\.js\?v=62-remove-promo-code/);
  assert.match(workerSource, /script\.js\?v=62-remove-promo-code/);
  assert.match(workerSource, /CACHE_NAME = "magicbook-pwa-v140-remove-promo-code"/);
});
