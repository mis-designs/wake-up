...
test("warm versioned files avoid network calls without changing bytes", async () => {
  const h = harness();
  const asset = "/learning-sync.js?v=6-local-review";
...
test("new versions, explicit reloads and unversioned assets still use the network", async () => {
  const h = harness();
  await h.dispatch("/learning-sync.js?v=6-local-review");
  await h.dispatch("/learning-sync.js?v=4-new-release");
  await h.dispatch("/learning-sync.js?v=6-local-review", { cache: "reload" });
  await h.dispatch("/learning-sync.js?v=6-local-review", { cache: "no-store" });
...
test("Cache Storage failures never break an online versioned resource", async () => {
  ...
  const response = await h.dispatch("/learning-sync.js?v=6-local-review");
