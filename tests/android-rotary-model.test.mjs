...
test("wheel numbers follow the finger and ascend from top to bottom", () => {
  ...
  assert.equal(dialLabelPosition(8, 8).x, 275);
  const dial = new ChapterDial(8);
  dial.begin(1, 0);
  const before = dialLabelPosition(9, 8);
  const result = dial.move(1, -33);
  const after = dialLabelPosition(9, result.preview);
  assert.equal(after.rotation - before.rotation, -33, 'number moves with the counterclockwise finger on the right side of the circle');
...
test("tapping the wheel selects the visible number without a drag or launch", () => {
  assert.equal(chapterAtAngle(8, 0), 8);
  assert.equal(chapterAtAngle(8, 33), 9);
  assert.equal(chapterAtAngle(8, -33), 7);
  assert.equal(chapterAtAngle(25, 33), 25);
  assert.equal(chapterAtAngle(1, -33), 1);
});
