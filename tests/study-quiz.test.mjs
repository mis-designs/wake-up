...
test("the study intro keeps Hadi Rounded only on the large title and uses Ekushey Lal Sabuj for smaller Bangla text", () => {
  ...
  const subtitleFont = readFileSync(new URL("../assets/fonts/ekushey-lal-sabuj/ekushey-lal-sabuj-regular.woff2", import.meta.url));
  const worker = readFileSync(new URL("../service-worker.js", import.meta.url), "utf8");
  const routes = JSON.parse(readFileSync(new URL("../vercel.json", import.meta.url), "utf8"));
  ...
  assert.doesNotMatch(page, /banglawebfonts\.pages\.dev\/css\/(?:hadi-rounded|ekushey-lal-sabuj)/u);
  ...
  assert.ok(subtitleFont.length > 50_000);
  ...
  assert.equal(createHash("sha256").update(subtitleFont).digest("hex"), "bdb7955ae89c44de417b9f894d0e09ba59a3ffe1a646c36a9b2bac7d6770b7a7");
