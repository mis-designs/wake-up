...
const QUIZ_PROXY_SECRET = SCRIPT_PROPS.getProperty("QUIZ_PROXY_SECRET");

function isProxyAuthorized(e, body) {
  const params = (e && e.parameter) ? e.parameter : {};
  const token = (body && body.token) || params.token;
  return token && token === QUIZ_PROXY_SECRET;
}
...
function doGet(e) {
  ...
  if (action === "getQuiz") {
    if (!isProxyAuthorized(e, null)) return jsonResponse({ error: "unauthorized" });
    return getQuiz(e);
  }
...
function getQuiz(e) {
  ...
  const modeParam = ((e && e.parameter && e.parameter.mode) || "").trim().toLowerCase();
  const requestedCount = getRequestedQuestionCount(e, modeParam);

  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("quiz");
  const data = sheet.getDataRange().getValues();
  const headers = data.shift();
  const columns = getQuizColumnMap(headers);

  let quiz = data.map(function(row) {
    return mapQuizRow(row, columns);
  }).filter(function(q) {
  ...
  if (chaptersParam) {
    ...
    if (selected.length > 0) {
      quiz = quiz.filter(function(q) {
        return selected.indexOf(String(q.chapter).trim()) !== -1;
      });
    }
  }
  ...
  // prendi il numero richiesto (default 30, exam80 80, exam30 30)
  const slice = quiz.slice(0, requestedCount);
...
function getRequestedQuestionCount(e, modeParam) {
  if (modeParam === "exam80") return 80;
  if (modeParam === "exam30") return 30;

  const params = (e && e.parameter) ? e.parameter : {};
  const candidates = [params.questionCount, params.count, params.limit];

  for (var i = 0; i < candidates.length; i++) {
    var n = Number(candidates[i]);
    if (!isNaN(n) && n > 0) return Math.min(Math.floor(n), 80);
  }

  return 30;
}

function getQuizColumnMap(headers) {
  var map = {};
  ...
  return {
    id: map.hasOwnProperty("id") ? map.id : 0,
    chapter: map.hasOwnProperty("chapter") ? map.chapter : 1,
    question: map.hasOwnProperty("question") ? map.question : 2,
    figure: map.hasOwnProperty("figure") ? map.figure : 4,
    correct: map.hasOwnProperty("correct") ? map.correct : 5,
    question_bd: map.hasOwnProperty("question_bd") ? map.question_bd : 6
  };
}
...
function mapQuizRow(row, columns) {
  return {
    id: getCell(row, columns.id),
    chapter: getCell(row, columns.chapter),
    question: getCell(row, columns.question),
    figure: getCell(row, columns.figure),
    correct: getCell(row, columns.correct),
    question_bd: getCell(row, columns.question_bd)
  };
}
...
function checkQuiz(e, body) {
  ...
  data.forEach(function(row) {
    var id = getCell(row, columns.id);
    var normalized = normalizeAnswer(getCell(row, columns.correct));
