...
- Public welcome: browser colors are white/forest green, not the installed Android blue palette. Keep the approved centered phone flow; from900px use information/actions on the left and a large intact floating book on the right. At normal zoom, the whole welcome including its sponsor must fit one viewport; use readable scrolling only when magnification or extreme height requires it. Verify the footer bounds, not only the action buttons. No Privacy link on this welcome and no animation pause/restart button. Always preserve the original MiskatDesigns logo as the existing clickable Facebook sponsor link, with a44px target and visible keyboard focus. Record these owner corrections in DESIGN.md/UX-CONTRACT.md before changing adjacent screens.
