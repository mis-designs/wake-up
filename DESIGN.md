...
design_context:
  ...
  revision_notes: The Android WebView now uses the Aura Folio premium system: porcelain canvas, plum chrome, one compact action folio, quieter elevation, semantic audio/status colors, and touch-first feedback. Ordinary browser presentation, route order, app logic, Admin identity, numberless figure delivery, and direct-manipulation chapter behavior remain intact.
---
...
- Installed-app presentation is owned by `android-app-theme.css` behind the `html.android-webview` marker. The Aura Folio direction uses plum chrome, a porcelain canvas, lilac utility surfaces, one white action folio with a slim purple spine, and restrained elevation. Red remains reserved for errors/danger and `NEW`; emerald remains the audio/positive color. The ordinary browser version keeps its current palette, while route structure, reading order, typography, and business behavior stay with their existing owners.

## Tokens

- Brand action: `--li-action: var(--color-primary)`; dark action `#263bd4`.
- Android Aura Folio palette: Aura purple `#5B1E91`, deep plum `#35104E`, raised violet `#73339B`, porcelain canvas `#F7F5FA`, white paper `#FFFFFF`, lilac surface `#BDB5E9`, soft lilac `#EFEAF5`, red signal/danger `#EB0000`, positive/audio emerald `#08785F`, primary ink `#111827`, muted ink `#4B5563`, and structural border `#AFAFB6`. Purple uses white text, lilac uses dark ink, and red remains a sparse signal rather than a competing primary action.
...
- Aura Folio Android shell: Home keeps its semantic action order—Magic Book, Dizionario, Statistiche, Errori—but groups it in one compact paper folio with a single plum spine. The Magic Book action is the only elevated primary action; supporting actions use quiet lilac or white surfaces. Capitoli groups the carousel, instrument controls, and Exam/Quiz actions in one reading surface. Quiz uses one paper question surface and one compact command dock. Coarse-pointer hover is neutralized and press feedback never changes control geometry; long routes keep one vertical scroll owner.
