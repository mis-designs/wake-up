...
- Composition: quiet white-to-light-gray Home, fixed 44px greeting viewport, original `icons/mg_book.svg`, no CSS shadow/filter. Book opens chapter selection. Chapter frame clips the left chapter illustration and right solid-black dial into opposing half-circles. A thin circular line (radius 100 in the 300-unit dial, 1.25px non-scaling stroke using `--native-line`) separates the complete opaque outer number band from the black interior. Selected 01–25 appears in black on a white rounded square inside the visible inner semicircle, centered at one-third of dial width and halfway down. Only the 64×44px Vai action remains centered in the gap between the two circles. The dial interior stays black around the number badge, not a white circular core. Four requested English-labelled controls remain Study Quiz, Do Quiz, Dictionary, Exam; supporting and accessible copy is Italian. Statistics and Errors follow the exact three supplied divider SVGs.
...
- Inner dial readout uses Norwester at `clamp(22px, 4dvh, 32px)`, in a square measuring the lesser of 25% of dial width or 64px. Corners use 35% of the side (70% of maximum rounding), preserving a rounded-square silhouette rather than the circle produced by a CSS 70% radius. Both digits and badge stay inside the separator even on the minimum 144px wheel. The badge reuses canvas white and wheel black; forced colors uses Canvas/CanvasText.
- Behavior: one captured pointer, 33-degree detents with a 0.08-step midpoint deadband, no inertia or wrapping, clamp 01–25, per-selection haptic and a double boundary pulse. Numbers ascend top-to-bottom and move with the finger, not against it; the outer selected value remains visible while the larger inner readout stays stationary. The owner removed separate previous/next buttons and visible rotation instructions. Tapping a number on the same wheel and Arrow/Home/End/Page keys remain non-drag alternatives; a gesture moving more than 6px cannot become a tap on release. No chapter opens on drag release/cancel. Resizing ends the current gesture. Canonical action gate rejects simultaneous route actions; returning from page cache restores the same selected chapter and dock.
...
- Motion: 5s gentle book float, restrained liquid wave. Home greets once per entry with `Assalamu alaikum`, the phone-local-hour greeting (05:00–11:59 Buongiorno, 12:00–17:59 Buon pomeriggio, otherwise Buonasera), then `Ciao!`; a 9s Norwester rail slides upward through one fixed line. The supplied `icons/clich_here.svg` rises toward the book, briefly compresses like a tap and fades, on a 10s cycle. The supplied bent up/down arrows travel around the wheel on a concentric orbit 28px outside its edge, fading along mirrored 18-to-32-degree arcs on a 6s cycle, with the lower cue delayed 0.8s. Their 22×26px images retain visible clearance from the black rim. They disappear after using the wheel. Gesture images are decorative and never intercept input. The optional animation control remains in Profile; reduced-motion/paused preference shows a static time-of-day greeting and gesture icons at mirrored 26-degree positions. Hidden/background pages pause motion. No written gesture hint on screen and no added book shadow.
...
- Chapter artwork: following the owner's request to recover card images from All Books, `assets/native-chapter-covers.json` explicitly maps all 25 chapter numbers to its public `quiz_cards/Capitolo_NN.png` illustrations. The All Books chapter catalog and Magic Book topic order establish the correspondence, including chapter 14, Ordine di precedenza / Precedenza e incroci. `scripts/build-native-chapter-covers.mjs` generates 640px-wide WebP copies (about 1.34MiB combined) without changing the originals. CSS frames the upper illustration in the semicircle, excluding the baked-in legacy title/action footer; no protected book pages are used. Only selected artwork is requested, with stale-load protection and a stable honest fallback for missing/failed images. Provenance and regeneration instructions live in `assets/chapter-covers/README.md`.
