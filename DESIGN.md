...
- Composition: quiet white-to-light-gray Home, Ciao heading, original `icons/mg_book.svg`, no CSS shadow/filter. Book opens chapter selection. Chapter frame clips the left photo and right dial into opposing half-circles; selected 01–25 sits in their gap. Inner white Vai action opens the selected book chapter. Four requested English-labelled controls remain Study Quiz, Do Quiz, Dictionary, Exam; supporting and accessible copy is Italian. Statistics and Errors follow the exact three supplied divider SVGs.
...
- Behavior: one captured pointer, 33-degree detents with a 0.08-step midpoint deadband, no inertia or wrapping, clamp 01–25, per-selection haptic and a double boundary pulse. Numbers ascend top-to-bottom and move with the finger, not against it; the selected value appears only in the gap. Arrow/Home/End/Page keys and 44px previous/next controls offer non-drag alternatives. No chapter opens on drag release/cancel. Resizing ends the current gesture. Canonical action gate rejects simultaneous route actions; returning from page cache restores the same selected chapter and dock.
...
- Motion: 5s gentle book float, restrained liquid wave. Per the owner's follow-up, Home has neither the tap instruction nor a pause label/button. The optional animation control lives in Profile and persists locally; reduced-motion is static and hidden/background pages pause. No added book shadow.
