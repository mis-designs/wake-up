...
- Behavior: one captured pointer, 33-degree detents, no inertia or wrapping, clamp 01–25, per-selection haptic and a double boundary pulse. Arrow/Home/End/Page keys and 44px previous/next controls offer non-drag alternatives. No chapter opens on drag release/cancel. Canonical action gate rejects simultaneous route actions.
...
- Motion: 5s gentle book float, restrained liquid wave. Home pause control persists locally; reduced-motion is static and hidden/background pages pause. No added book shadow.
...
- Layout: main chapter canvas max540px, intentionally cropped circles only; one scrolling content area reserves dock space. Small/short viewports scroll vertically without overlapping actions. CSS scope never changes normal website geometry.
