...
- Composition: quiet white-to-light-gray Home, fixed 44px greeting viewport, original `icons/mg_book.svg`, no CSS shadow/filter. Book opens chapter selection. Chapter frame clips the left photo and right solid-black dial into opposing half-circles. A thin circular line (radius 100 in the 300-unit dial, 1.25px non-scaling stroke using `--native-line`) separates the complete opaque outer number band from the black interior. Selected 01–25 appears in white inside the visible inner semicircle, in a stable 44px-high slot centered at one-third of dial width and one-third wide. Only the 64×44px Vai action remains centered in the gap between the two circles. No white dial core. Four requested English-labelled controls remain Study Quiz, Do Quiz, Dictionary, Exam; supporting and accessible copy is Italian. Statistics and Errors follow the exact three supplied divider SVGs.
...
- Inner dial readout uses 28–40px Norwester, capped at 28px for viewport heights up to 620px so both digits remain inside the separator even on the minimum 144px wheel.
