...
design_context:
  ...
  revision_notes: Owner-approved transparent chapter arc and pearl/graphite/blue installed-app palette replace the black wheel and lavender theme. Section-only compact headers, title-to-artwork transitions and gesture cues remain Android-only. Existing routes, authorization and progress data remain canonical.
---
...
- Evidence: owner-supplied 2026-09-08 mockups and explicit follow-up instructions. The latest transparent arc and pearl/blue reference supersede the former black wheel, badge and lavender palette. Half-circle cropping, the supplied traffic divider, soft book motion and liquid progress remain approved for the study shell. Other Android routes receive only the requested palette and compact section-header variant.
...
- Composition: pearl Home, fixed 44px greeting viewport, original `icons/mg_book.svg`, no CSS shadow/filter. Book opens chapter selection. A transparent numbered arc is anchored beyond the left edge, matching the latest reference. Its thin circular divider uses the decorative divider token; the sole selected 01–25 value is blue. A fixed frame to the right presents the title, then the chapter illustration. The 72×44px Vai button stays beneath this frame. There is no permanent image semicircle, black wheel, duplicate readout or number badge. The six established actions and original traffic divider stay above the floating dock.
...
- Type: self-hosted, unmodified Norwester v1.2 for short Latin display/action labels; self-hosted El Messiri variable 500 for the owner's ornamental chapter-title direction, with Georgia/serif fallback. Latin names remain unchanged. Both licenses accompany their assets; no runtime third-party font service. Arial/system remains smaller explanatory copy.
...
- Chapter artwork: the existing manifest maps all 25 topics to optimized 640px-wide All Books public cover illustrations; originals are unchanged. Only the selected image loads after a 120ms idle delay. The title remains for at least 1.8s, then decoded artwork replaces it with a 260–300ms fade in the same frame. Missing/slow images leave the real title visible. Selection, dragging, hiding or navigation invalidates pending reveals; stale artwork cannot replace a newer title. The accessible title stays available while decorative artwork is shown. Paused animation keeps the title. No permanent image semicircle or missing-image placeholder. CSS crops the legacy footer, and provenance remains in `assets/chapter-covers/README.md`.
- Layout: max540px chapter canvas; only the off-edge circular arc is intentionally cropped. The sole content scroller ends 12px above the measured dock, including safe-area changes. The stage has a 210px minimum row and uses remaining height; the dial is bounded by min(78vw,421px). Title/artwork share one absolute frame; El Messiri adapts locally from 28px to 16px, cached and remeasured after font readiness. Long/enlarged titles may scroll within that frame but never move surrounding geometry. Vai remains below it. Six controls retain 48/64px heights, compact 44/52px below620px; short windows/landscape scroll only above the dock. Ordinary browser geometry is unchanged.
