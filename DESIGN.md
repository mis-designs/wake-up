...
- Evidence: owner-supplied 2026-09-08 mockups and follow-ups. The latest written request overrides the screenshot: transparent wheel on the right, complete rounded card on the left. The pearl/blue palette, supplied traffic divider, soft book motion and liquid progress remain approved. Other Android routes are unchanged in this revision.
...
- Chapter artwork: the existing manifest maps all25 topics to optimized 640px-wide All Books covers; originals are unchanged. Only the selected image loads after120ms idle. The title stays for at least1.8s, then decoded artwork replaces it with a260–300ms fade. Missing/slow images keep the real title. Selection, drag, hiding and navigation invalidate pending reveals. The accessible title stays available with decorative artwork; explicit pause keeps the title. The full card, including its original artwork footer, uses bounded intrinsic dimensions, `object-fit: contain` and16px rounded corners. No crop or image semicircle. Provenance: `assets/chapter-covers/README.md`.
