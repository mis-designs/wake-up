...
design_context:
  ...
  revision_notes: The Android WebView now uses the Roadcraft system: a compact top-led flow, blue navigation, green study action, midnight dictionary, lime exam cue, open chapter cockpit, restrained elevation, and touch-first feedback. Ordinary browser presentation, route order, app logic, Admin identity, numberless figure delivery, and direct-manipulation chapter behavior remain intact.
---
...
- Android WebView compactness is owned by `mobile-experience.css`; it may reduce decoration and spacing without changing route meaning or hiding required controls. Header titles stay visually centered through symmetric live utility rails. Home and Capitoli begin near the header in normal flow, never use vertical auto-centering, and keep one natural scroll owner for shorter screens.
- Installed-app presentation is owned by `android-app-theme.css` behind the `html.android-webview` marker. Roadcraft uses blue only for orientation and primary navigation, emerald for the main study action and audio, midnight for the dictionary, lime for the Exam cue, white for supporting content, and red only for errors/danger and `NEW`. Home and Capitoli stay visually open: no giant enclosing folio, decorative spine, or card around an existing group of cards. The ordinary browser version keeps its current palette, while route structure, reading order, typography, and business behavior stay with their existing owners.

## Tokens

- Brand action: `--li-action: var(--color-primary)`; dark action `#263bd4`.
- Android Roadcraft palette: navigation blue `#315EFB`, strong blue `#243BC7`, deep navy `#17233A`, mist canvas `#F4F7FB`, white paper `#FFFFFF`, soft blue `#EAF0FF`, signal lime `#C9F41D`, danger red `#D92D42`, positive/audio emerald `#087A60`, muted ink `#637083`, and structural border `#D7DEEA`. Blue and emerald use white text; lime uses navy text; red remains a sparse signal rather than a competing primary action.
...
- Chapter interaction: the installed-app carousel is the single expressive motion moment. While dragging, cards continuously scale and fade around the pointer position; edges add resistance and release velocity projects at most six chapter steps before the track settles. Vertical page scrolling remains native. Every card is a real roving-tabindex button, tap remains available, Arrow/Home/End provide keyboard selection, Enter/Space opens the selected chapter, cancellation never launches content, and reduced-motion removes the spring timing.
- Roadcraft Android shell: Home keeps its semantic action order—Magic Book, Dizionario, Statistiche, Errori—in a compact top-led cluster without an enclosing card. Magic Book is the elevated emerald primary action; the midnight dictionary and paired white learning actions remain distinct. Capitoli presents title, carousel, instrument cluster, and Exam/Quiz actions directly on the page without a second enclosing surface. Admin retains its established green, violet, amber, red, and slate distinctions at a denser app-only scale. Quiz uses one paper question surface and one compact command dock. Coarse-pointer hover is neutralized and press feedback never changes control geometry; long routes keep one vertical scroll owner.
