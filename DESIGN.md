...
- Explanation audio player: `audio-player-ui.css` is the shared visual owner for Quiz and Studia quiz. It carries the Admin pill language into one light player surface: a 40px emerald radial play/pause control, emerald progress, violet speed utility, stable loading/error geometry, cobalt focus, and motion-safe hover feedback. Play and pause marks are geometrically centred rather than baseline-aligned.
