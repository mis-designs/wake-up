...
design_context:
  ...
  revision_notes: Owner-approved right-hand transparent wheel with blue marker and inner Vai, complete left-hand card, six bilingual action rails and variable roundabout motion. Android-only; existing routes, authorization, progress data and normal website remain canonical.
---
...
- Installed-app color is owned by `android-app-theme.css` behind the `html.android-webview` marker. Pearl chrome, graphite controls and blue selection/actions follow the owner's latest palette reference; white reading paper stays distinct. Ordinary browsers keep their existing colors and geometry. Thin section-only Android headers are an explicit shared theme variant; business behavior and route structure remain unchanged.
...
- Token mapping (model B): `android-app-theme.css` owns `--app-palette-*`, maps them to existing route aliases and shared components, and supplies theme-color metadata through `android-webview-mode.js`. Native study tokens alias this owner instead of duplicating colors. The `aura-fluid` dataset remains a compatibility switch, not the palette source. Only the documented Android header/rotary variants change density, typography and motion.
...
- Scope: Home and chapter templates mount only in `html.android-webview`. Desktop/mobile browsers and installed browser PWAs never mount them. Other Android routes receive the shared palette and thinner section-only header, not a new route layout.
...
- Motion: 5s gentle book float and restrained liquid wave. Home greets once with `Assalamu alaikum`, the phone-local-hour greeting (05:00–11:59 Buongiorno, 12:00–17:59 Buon pomeriggio, otherwise Buonasera), then `Ciao!`, in a fixed 9s upward rail. The supplied hand repeats every 5s, starts black below the book, becomes white over the artwork, compresses and fades. Bent-arrow cues move along mirrored 24–40-degree arcs every 3.8s, lower cue delayed .45s, outside the numbered orbit; they fade during interaction and return after 4.5s idle. Decorative cues never intercept input. Profile motion preference and system reduced-motion remove motion; background/navigation cancels transient work. No instructional copy or added book shadow.
...
- Layout: max540px canvas; only the off-edge circular arc is intentionally cropped. The sole scroller ends12px above the measured dock. The stage has a210px minimum and uses remaining height; the dial is bounded by min(78vw,421px). Title/artwork share one fixed frame; El Messiri fits locally from28px to16px, remeasured after font readiness. Enlarged titles may scroll within that frame but never move surrounding geometry. Vai follows wheel geometry and fits completely inside its line. Six controls retain48/64px heights, compact44/52px below620px; landscape scrolls above the dock. Normal browser geometry is unchanged.
