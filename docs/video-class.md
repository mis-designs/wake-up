...
### Playback continuity — 26 September2026

The owner approved keeping a lesson open across notifications/app switches and resuming at the last actual YouTube playhead. The same bounded v1 local record now includes optional `position` and `positionUpdatedAt`; old records retain coverage but do not invent a resume point. Latest position wins independently from cumulative viewed ranges, including backward seeks and zero. The user-initiated embed receives validated `start` seconds; onReady also calls the official `seekTo` for the saved fractional position. The existing poster says `Riprendi da m:ss`. Provider-ended resets the next start to zero, never sets watched coverage to100%.

Hidden/native-unfocused/bfcache pages pause and flush without destroying the player or rerendering the lesson. Return leaves it paused. Explicit navigation samples before invalidating/removing it; account guards reject stale observations. Delayed readiness while hidden cannot autoplay or overwrite the checkpoint with an unstarted zero. The one-second timer still exists only during foreground playback; five-second persistence and immediate pause/navigation/pagehide saves remain local, with no new server calls. Sudden OS termination may lose the last few seconds; cleared/blocked storage and external YouTube/Facebook playback cannot offer durable measured resume.

The native shell preserves its custom video view in the background. Activity lifecycle/window-focus events notify the exact trusted document through `VideoPlaybackPolicy.kt` (origin rechecked inside the script), covering notification-shade focus loss without treating ordinary browser iframe focus as an interruption. Existing Back, navigation, recovery, disposal and book capture protection remain unchanged. This native part needs a new installed Android build, not only a website update.

Verification26 September:686 web unit/regression tests pass with test concurrency2; Android20 unit tests and debug APK assembly pass offline. Strict premium audit and official DESIGN lint report no findings. Local browser fixtures pass at375px web/native,1440px web and740px native landscape: actual frame identity across visibility/native/bfcache interruptions, paused foreground return, backward-seek checkpoint, persisted reload/start parameter, unchanged coverage, quota fallback, account separation and unchanged catalog-read counts. Resume screenshots were inspected on phone and desktop. These playback runs set `QA_SKIP_INDICATOR_TIMING=1`: the independent real AudioContext timing subcase used the existing silent fallback in headless testing, so its successful-audio timing was not certified by this run; indicator unit/failure-fallback checks remain covered and its implementation is unchanged. No connected Android device was available. Live YouTube and physical notification-shade/fullscreen acceptance remain required before release. No production account or provider traffic was used for browser verification.
