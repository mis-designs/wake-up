...
| Android installed palette | `android-webview-mode.js`, `android-app-theme.css` | `html.android-webview` and `data-app-palette="aura-fluid"` | Palette roles documented in DESIGN.md: lavender primary, gold secondary/accent, ivory background, sage surface, dark ink, and distinct neutral borders; browser palette and semantic status colors remain unchanged | Early-marker order, route coverage, offline cache, scope, contrast, and reduced-motion tests |
...
| Android app-only theme | `android-webview-mode.js`, `android-app-theme.css` | The shared synchronous marker runs before route stylesheets and activates the installed palette only for the Android WebView user agent. Its existing aura-fluid dataset value preserves the chapter drag contract. Theme-color metadata reads the primary token from CSS. The final stylesheet maps supplied palette tokens across Home, Capitoli, Quiz, Studia quiz, Dizionario, Statistiche/Errori, Privacy, and Admin utilities without changing route layout. Correct, incorrect, warning, loading, and failure states keep their established semantic colors. |
