...
| Android native shell | `mobile-experience.css`, `script.js` + Android `MainActivity.kt` | The shell fixes browser text zoom at 100 percent and marks its document as `android-webview`. Home omits the unreliable remote promo frame; its action cluster centers when it fits and becomes a top-safe natural scroll when it does not. The header derives symmetric side rails from visible Admin, Profile, and menu utilities so the title never sits beneath an icon. Essential actions retain usable targets and one transition owner rejects competing launch taps. The chapter carousel batches pointer work once per frame, adds resistance only at its real boundaries, projects a bounded release, and snaps without launching after cancellation. Cards are semantic buttons with roving focus; tap and Arrow/Home/End remain non-drag alternatives. |
