...
| Live quiz bilingual help | `quiz-help.js`, `quiz-help.css`, `magic-styles.css`, `#quiz-help-workspace` | Authenticated question and shared help resolver | Inline disclosure below the question; text keyword pills expand meanings in the same panel | Content order, keyboard disclosure, stale response, close/reopen, narrow layout and readable glass checks |
...
| Explanation audio player | `audio-focus.js`, `audio-player-ui.css` + owning playback JS | Shared Quiz/Studia quiz transparent emerald pill using only the approved play/pause SVG pair | Idle, playing, TTS-interrupted, loading, unavailable/error; 1x to first-click 0.5x, then up to 2x | No overlap, latest-action race tests, timestamp-preserving resume, manual-stop suppression, bounded 320px geometry, ARIA label, keyboard, reduced-motion, forced-colors, and cache tests |
...
| Android WebView density | `mobile-experience.css`, `script.js`, Android `MainActivity.kt` | Standard Android WebView marker plus live utility-count state | Compact top-led Home actions; centered collision-free header with compact utility rails; open chapter selector and Quiz command dock; denser touch-safe Admin scrolling; ordinary browser layout remains unchanged | Native marker, utility rails, top-flow, 320px paired actions, Admin scroll ownership, touch target, drag-frame, cancellation, keyboard alternative, exclusive-action, and Android build checks |
| Android installed palette | `android-webview-mode.js`, `android-app-theme.css` | `html.android-webview` and the legacy interaction marker `data-app-palette="aura-fluid"` | Roadcraft: blue orientation, emerald study/audio, midnight dictionary, lime Exam cue, white supporting surfaces, dark ink, and sparse red signals; browser palette and semantic status colors remain unchanged | Early-marker order, route coverage, offline cache, scope, contrast, global scrollbar, coarse-pointer, forced-colors, and reduced-motion tests |
...
| Live quiz bilingual help | `quiz-help.js`, `quiz-help.css`, `magic-styles.css`, `italian-display.js` | The native question button toggles a glass region directly below the question. Translation, real keyword text pills and inline word details share one natural-height surface; optional context follows. No modal, backdrop, focus trap or inert quiz surface. |
...
| Android native shell | `mobile-experience.css`, `script.js` + Android `MainActivity.kt` | The shell fixes browser text zoom at 100 percent and marks its document as `android-webview`. Home omits the unreliable remote promo frame and places its compact action cluster near the header in normal flow. Statistiche/Errori remain paired down to 320px, with smaller internal geometry instead of a structural reorder. The Admin route owns one full-height vertical scroller with native touch momentum and uses denser app-only controls, while the body remains fixed and ordinary browsers retain document scrolling. The header derives compact symmetric side rails from visible Admin, Profile, and menu utilities so the title never sits beneath an icon. Essential actions retain usable targets and one transition owner rejects competing launch taps. The chapter carousel batches pointer work once per frame, adds resistance only at its real boundaries, projects a bounded release, and snaps without launching after cancellation. Cards are semantic buttons with roving focus; tap and Arrow/Home/End remain non-drag alternatives. |
| Android app-only theme | `android-webview-mode.js`, `android-app-theme.css` | The shared synchronous marker runs before route stylesheets and activates Roadcraft only for the Android WebView user agent. The final stylesheet maps the system across Home, Capitoli, Quiz, Studia quiz, Dizionario, Statistiche/Errori, Privacy, Admin, and shared scroll surfaces without changing route meaning or data behavior. Home and Capitoli remain open compositions without an outer folio or nested panel; the instrument cluster and direct-manipulation chapter carousel are the signature treatment. Correct, incorrect, warning, loading, and failure states keep their established semantic colors, while explanation audio remains emerald. |
...
- In Android WebView, the Home action cluster begins near the header in normal flow and the existing vertical scroller owns shorter-height overflow. Statistiche and Errori remain paired at 320px with compact internal sizing. Capitoli follows the same top-led rhythm and never inserts a flexible blank region before the carousel. The fixed Admin/Profile utilities and in-header menu receive explicit right-edge slots, while equal left/right header rails keep the route title centered.
