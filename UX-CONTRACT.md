...
| Android WebView density | `mobile-experience.css`, `script.js`, Android `MainActivity.kt` | Standard Android WebView marker plus live utility-count state | Compact, vertically balanced Home folio; centered collision-free header with compact utility rails; fluid chapter selector and Quiz command dock; touch-safe Admin scrolling; ordinary browser layout remains unchanged | Native marker, utility rails, auto-margin overflow, 320px Home fallback, Admin scroll ownership, touch target, drag-frame, cancellation, keyboard alternative, exclusive-action, and Android build checks |
| Android installed palette | `android-webview-mode.js`, `android-app-theme.css` | `html.android-webview` and `data-app-palette="aura-fluid"` | Aura Folio: plum chrome, porcelain canvas, white paper, lilac support surfaces, dark ink, emerald audio/positive feedback, and sparse red signals; browser palette and semantic status colors remain unchanged | Early-marker order, route coverage, offline cache, scope, contrast, global scrollbar, coarse-pointer, forced-colors, and reduced-motion tests |
...
| Android native shell | `mobile-experience.css`, `script.js` + Android `MainActivity.kt` | The shell fixes browser text zoom at 100 percent and marks its document as `android-webview`. Home omits the unreliable remote promo frame; its folio centers when it fits and becomes a top-safe natural scroll when it does not. At 351px and wider Statistiche/Errori remain paired; at 350px and below they stack without horizontal clipping. The Admin route owns one full-height vertical scroller with native touch momentum, while the body remains fixed and ordinary browsers retain document scrolling. The header derives compact symmetric side rails from visible Admin, Profile, and menu utilities so the title never sits beneath an icon. Essential actions retain usable targets and one transition owner rejects competing launch taps. The chapter carousel batches pointer work once per frame, adds resistance only at its real boundaries, projects a bounded release, and snaps without launching after cancellation. Cards are semantic buttons with roving focus; tap and Arrow/Home/End remain non-drag alternatives. |
| Android app-only theme | `android-webview-mode.js`, `android-app-theme.css` | The shared synchronous marker runs before route stylesheets and activates Aura Folio only for the Android WebView user agent. The final stylesheet maps the system across Home, Capitoli, Quiz, Studia quiz, Dizionario, Statistiche/Errori, Privacy, Admin, and shared scroll surfaces without changing route meaning or data behavior. The Home folio spine and chapter carousel are the only signature treatments; Quiz uses one question surface and one command dock. Correct, incorrect, warning, loading, and failure states keep their established semantic colors, while explanation audio remains emerald. |
