...
| Android WebView density | `mobile-experience.css`, `script.js`, Android `MainActivity.kt` | Standard Android WebView marker plus live utility-count state | Compact, vertically balanced Home; centered collision-free header; chapter selector and Quiz; ordinary browser layout remains unchanged | Native marker, utility rails, auto-margin overflow, touch target, drag-frame, exclusive-action, and Android build checks |
| Android installed palette | `android-webview-mode.js`, `android-app-theme.css` | `html.android-webview` and `data-app-palette="quantum-signal"` | Graphite chrome, signal-orange actions, pavement-gray canvas, white reading surfaces; browser palette and semantic status colors remain unchanged | Early-marker order, route coverage, offline cache, scope, and contrast tests |
...
| Android native shell | `mobile-experience.css`, `script.js` + Android `MainActivity.kt` | The shell fixes browser text zoom at 100 percent and marks its document as `android-webview`. Home omits the unreliable remote promo frame; its action cluster centers when it fits and becomes a top-safe natural scroll when it does not. The header derives symmetric side rails from visible Admin, Profile, and menu utilities so the title never sits beneath an icon. Essential actions retain usable targets, chapter drag batches once per frame, and one transition owner rejects competing launch taps. |
| Android app-only theme | `android-webview-mode.js`, `android-app-theme.css` | The shared synchronous marker runs before route stylesheets and activates the theme only for the Android WebView user agent. The final stylesheet maps supplied palette tokens across Home, Capitoli, Quiz, Studia quiz, Dizionario, Statistiche/Errori, Privacy, and Admin utilities without changing layout. Correct, incorrect, warning, loading, and failure states keep their established semantic colors. |
